...
def main():
    torch.manual_seed(123); dev = "cuda" if torch.cuda.is_available() else "cpu"
    cfg = dict(X0_CFG, hidden_dim=96, ffn_dim=256, memory_slots=4, scratch_size=8)
    m = AetherX(128, cfg).to(dev).eval(); ids = torch.randint(0, 128, (2, 31), device=dev)
    ok = True
    with torch.no_grad():
    ...
    ok &= check("estado constante con 4 o 31 tokens",
                all(x.shape == y.shape for x, y in zip(a.__dict__.values(), b.__dict__.values())))

    mt = AetherX(128, cfg).to(dev).train(); logits = mt(ids)
    loss = lm_loss(logits[:, :-1], ids[:, 1:]) + 1e-3 * mt.cell.route_penalty(); loss.backward()
    ok &= check("loss finita", bool(torch.isfinite(loss)))
    ok &= check("embedding y celda reciben gradiente", all(p.grad is not None for p in mt.parameters() if p is not mt.cell.halt.weight and p is not mt.cell.halt.bias))
    ok &= check("router halt recibe gradiente", mt.cell.halt.weight.grad is not None)

    with torch.no_grad():
        m.cell.halt.bias.fill_(-10.0); z, st = m(ids[:, :5], m.init_state(2, dev), return_state=True, adaptive=True)
    ...
    with tempfile.TemporaryDirectory() as td:
        p = Path(td) / "state.pt"; torch.save({k: v.cpu() for k, v in st.__dict__.items()}, p)
        raw = torch.load(p, weights_only=False); ok &= check("estado serializable", raw["slots"].shape == st.slots.shape)
